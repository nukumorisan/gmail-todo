...
| `TASK_TITLE_PREFIX` | `null` | `null` の場合は付けない。指定時はTODOタイトルの先頭に付ける |
...
過去のバージョンで `TASK_LIST_TITLE` または `TASK_TITLE_PREFIX` が空文字として保存されている場合は、`setup()` が同じ意味の `null` へ自動移行します。その他の既存値は上書きしません。
...
- 存在する利用者向け設定は上書きせず、不足している項目だけを初期値で追加する。ただし、旧形式の空文字は同じ意味の `null` へ移行する
...
スクリプトプロパティの `TASK_LIST_TITLE` をGoogle Tasks上のタスクリスト名と完全一致させるか、最初のタスクリストを使用する場合は `null` に戻します。
