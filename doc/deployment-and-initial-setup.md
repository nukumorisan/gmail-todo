...
1. Gemini APIキーを確認する
2. Google Tasksのタスクリストを確認する
3. マイドライブに除外メールログ用スプレッドシートを作成または再利用する
4. 旧形式の除外メールログがあれば移行する
5. `processEmails()` を5分おきに実行するトリガーを作成する
...
`EXCLUDED_EMAIL_SPREADSHEET_ID` は手動で追加しないでください。`setup()` が除外メールログ用スプレッドシートを作成した後、自動的に保存します。

## 9. 必要に応じて設定を変更する

初期値のまま利用する場合、この手順は不要です。変更する場合は、`コード.gs` 冒頭の `CONFIG` を編集します。

主な設定は次のとおりです。

| 設定 | 初期値 | 内容 |
|---|---:|---|
| `EXCLUDED_SUBJECT_KEYWORDS` | 複数の除外語 | 件名だけで除外するキーワード |
| `GMAIL_SEARCH_QUERY` | `in:inbox newer_than:2d` | Gmailの検索条件 |
| `MAX_THREADS_PER_RUN` | `30` | Gmail検索をページ送りするときに1回で取得するスレッド数 |
| `MAX_GEMINI_REQUESTS_PER_RUN` | `15` | `processEmails()` 1回あたりのGemini API最大呼び出し回数 |
| `TASK_LIST_TITLE` | `null` | `null` の場合は最初のタスクリストを使用。指定時はGoogle Tasks上の名前と完全一致させる |
...
設定を変更したら保存してください。`TRIGGER_INTERVAL_MINUTES` を変更した場合は、後述の `setup()` を再実行するとトリガーへ反映されます。
...
「プロジェクトの設定」→「スクリプト プロパティ」に `EXCLUDED_EMAIL_SPREADSHEET_ID` が自動追加されていることを確認します。この値は通常、変更や削除をしません。
...
`CONFIG.TASK_LIST_TITLE` をGoogle Tasks上のタスクリスト名と完全一致させるか、最初のタスクリストを使用する場合は `null` に戻します。
